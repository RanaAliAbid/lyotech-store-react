...
export const verifyUserHandover = async (params: PartnerLinkData) => {
    try {
        return ProxyService.PostRequest(PROXY_HOST + "/api/v1/auth/checktoken", params);
    } catch (error) {
        return null
    }
}
...
export const validateUserEmailOtp = async (token: string, otp: string, keyToken: string) => {
    try {
        const params = {token: token, otp: parseInt(otp)}
        return ProxyService.PostRequest(PROXY_HOST + "/api/v1/auth/verify-email-otp", params, keyToken);
    } catch (error) {
        return null
    }
}
...
export const fogotPasswordUser = async (params: ForgotPasswordData) => {
    try {
        return ProxyService.PostRequest(PROXY_HOST + "/api/v1/auth/forgot-password", params);
    } catch (error) {
        return null
    }
}
...
export const changePasswordUser = async (params: ForgotPasswordData) => {
    try {
        return ProxyService.PostRequest(PROXY_HOST + "/api/v1/auth/change-password", params);
    } catch (error) {
        return null
    }
}