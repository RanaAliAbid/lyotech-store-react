import { SignInData, SignInDataValidator, SignUpData, SignUpDataValidator } from "@/components/auth/auth.types";
...
export const signUpCheckEmptyFields = (data: SignUpData): SignUpDataValidator => {
    const dataValidate: SignUpDataValidator = { firstName: true, lastName: true, email: true, password: true, password_confirm: true }
    ...
    dataValidate.password_confirm = (data.password_confirm != data.password) ? false : true

    return dataValidate;
}

export const signInCheckEmptyFields = (data: SignInData): SignInDataValidator => {
