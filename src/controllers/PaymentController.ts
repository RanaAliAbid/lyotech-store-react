import { ApiData, ApiError } from "@/pages/api/types";
import { NextApiRequest, NextApiResponse } from "next";
import { API_HOST } from "../../app.config";
import { ApiService } from "@/services/api.service";

export const getPaymentMethods = async (
    req: NextApiRequest,
    res: NextApiResponse<ApiData | ApiError>
) => {
    res.setHeader('Allow', 'GET');

    try {
        let data = req.body;

        const result = await ApiService.GetRequest(API_HOST + '/v1/payment/payment-method', `Bearer ${req.cookies?.authToken}`);

        res.status(200).json(ApiService.ApiResponseSuccess(result?.data?.data, ''));

    } catch (error: any) {
        console.log('Catch error payment method ', error?.response?.data);
        res.status(400).json(ApiService.ApiResponseError(error));
    }
};