...
export const userCart = async (
  ...
  try {
    ...
    if (req.cookies?.guestId) {
      res.setHeader('Set-Cookie', [`guestId=""; HttpOnly; Max-Age=0;`]);
    }
...
export const userAddToCart = async (
  ...
  try {
    ...
    if (error?.response?.data?.code == '401') {
      res.setHeader('Set-Cookie', [`guestId=deleted; HttpOnly; Max-Age=0;`]);
    }
  ...
  try {
    ...
    if (token) {
      result = await ApiService.PutRequest(
        API_HOST + '/v1/cart/remove',
        data,
        `Bearer ${token}`
      );
    } else {
      result = await ApiService.PutRequest(
        API_HOST + '/v1/cart/guest/remove',
        data,
        `${guestId}`,
        true
      );
...
export const userEmptyCart = async (
  ...
  try {
    ...
    if (token) {
      result = await ApiService.DeleteRequest(
        API_HOST + '/v1/cart',
        `Bearer ${token}`
      );
    } else {
...
export const cartShippingMethods = async (
  ...
  try {
    const result = await ApiService.GetRequest(
      API_HOST + '/v1/shipping/shipping-method',
      `Bearer ${req.cookies?.authToken}`
    );
...
export const cartUpdate = async (
  ...
  try {
    ...
    if (token) {
      result = await ApiService.PutRequest(
        API_HOST + '/v1/cart/update',
        data,
        `Bearer ${token}`
      );
    } else {
      result = await ApiService.PutRequest(
        API_HOST + '/v1/cart/guest/update',
        data,
        `${guestId}`,
        true
      );
...
export const cartCurrencyRate = async (
  req: NextApiRequest,
  res: NextApiResponse<ApiData | ApiError>
) => {
  res.setHeader('Allow', 'POST');
  ...
  try {
    ...
    let result = await ApiService.GetRequest(
      `${API_HOST}/v1/currency/get-conversion-rate/${requestParams}`,
      ``
    );
