...
export default function CartTotalComponent({
  ...
  React.useEffect(() => {
    if (
      termsCheckbox.presale &&
      termsCheckbox.onecare &&
      termsCheckbox.minting
    ) {
  ...
  const handleChangeShippingType = async (
    ...
    try {
      ...
      const result = await globalContext.updateCartShippingMethod(
        shippingMethodId
      );
  ...
  const handleClickShippingType = async (shippingMethodId: string) => {
    try {
      globalContext.setGlobalLoading(true);
  ...
  const updateCartOneCarePolicy = async (
    status: boolean,
    id: string,
    force = false
  ) => {
    ...
    <div className="position-relative">
      ...
      <div className={styles.wrapBox}>
        <div className={`${styles.summaryDetails}`}>
          <List>
            <ListItem className={styles.subTotal}>
              ...
              <Typography variant="h6">
                {(
                  globalContext?.cart?.cart?.totalAmount *
                  ...
                  <ListItem key={index} className={styles.productsLists}>
                    <div className={styles.productsData}>
                      ...
                      <div className={styles.productInfo}>
                        ...
                        <Typography variant="h6">
                          {' '}
                          Model Name: {cartItem?.productId?.name}
                        </Typography>
                        {globalContext?.cart?.cart?.totalAmount == 0 ? (
                          <Typography variant="h5">
                            ...
                            <b>
                              {(parseFloat(
                                cartItem?.productId?.price?.toFixed(
                                  globalContext.priceToFixed
                                )
                              ) -
                                parseFloat(
                                  globalContext.cart?.cart?.coupon?.find(
                                    (x: any) =>
                                      x.productId == cartItem?.productId?._id
                                  )?.couponId?.amount
                                )) *
                                globalContext.conversionRate}
                            </b>
                ...
                (item: any, index: any) =>
                  ...
                  !['oneCare', 'payment'].includes(item.type) && (
                    <ListItem key={index}>
                      ...
                      <Typography variant="h6">
                        {(item?.fee * globalContext.conversionRate)?.toFixed(
                          globalContext.priceToFixed
                        )}{' '}
                        {globalContext.currencySymbol}
                      </Typography>
                        ...
                        <Checkbox
                          ...
                          checked={
                            globalContext?.cart?.cart?.oneCare?.findIndex(
                              (x: any) =>
                                x.productId ==
                                globalContext?.cart?.cart?.products[0]
                                  ?.productId?._id
                            ) !== -1
                              ? true
                              : false
                          ...
                          onChange={(e) =>
                            updateCartOneCarePolicy(
                              e.target.checked,
                              globalContext?.cart?.cart?.products[0]?.productId
                                ?._id ?? ''
                            )
                          }
                          size="small"
          ...
          <List>
            <ListItem className={styles.summaryfoot}>
              ...
              <Typography variant="h5">
                ...
                {(
                  globalContext?.cart?.cart?.totalIncludingFees *
                  globalContext.conversionRate
                )?.toFixed(globalContext.priceToFixed)}{' '}
                {globalContext.currencySymbol}
              </Typography>
            ...
            <>
              <div className={`${styles.wrapTitle} mt-2 mb-2`}>
                <DefaultAddressComponent
                  ...
                  setChangeAddress={addressProps?.setChangeAddress}
                ></DefaultAddressComponent>
              </div>

              <div className={`${styles.wrapTitle} mt-2 mb-2`}>
                <Typography variant="h4">Choose a payment method</Typography>
              </div>
        ...
        {isCheckout ? (
          ...
          <Button
            onClick={(e) => {
              globalContext.cart?.cart?.products?.length > 0 &&
                router.push(`/${router.locale}/checkout`);
            }}
            variant="contained"
            fullWidth
            className={`${styles['btn']} ${styles['btn_primary']}`}
      ...
      {showOneCareModal && (
        <div className="oneCarePolicyModal">
          <Typography variant="h4" className="bold-500">
            {t('One Care Policy')}
          </Typography>
          <br />
          <Typography variant="h6">
            <p>
              Your phone is susceptible to physical damage and unexpected
              accidents can happen at any time.
            </p>
            <br />
            <p>
              One Care offers comprehensive protection that covers you against
              these unforeseen damages.
            </p>
            <br />
            <p>
              Don't take the risk of expensive repairs or replacements - ensure
              that you're protected with One Care today.
            </p>
            <br />
            <div className="">
              <Button
        ...
        <Box sx={{ ...modalStyle, maxWidth: '500px', width: '100%' }}>
          ...
          <ListItem>
            <div className={styles.allCenter}>
              ...
              <Typography variant="h6" className="t-13">
                I have read and Accept{' '}
                <Link rel="noreferrer" target="_blank" href={`/onecare-policy`}>
                  <span className="text-primary cursor-pointer">
                    One Care Policy
                  </span>
                </Link>
